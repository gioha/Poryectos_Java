 ...
 * @version 1.0
 * @since 21/04/2017 11:00:57
 */
@Controller("mbCapActaDRP")
@Scope("session")
public class MBCapturaVotoDiputadoRP extends MBGeneric implements Serializable {

	private static final long serialVersionUID = -7016360748596285429L;

	private static final Log log = LogFactory.getLog(MBCapturaVotoDiputadoRP.class);
	...
	public void constructorForm() {
		log.info("Entro a MBCapturaVotoDiputadoRP.constructorForm" );
		try {
			form = new FormRegistroActa();
			form.setActa(new DTOActa());
			candidatura = hlp.getTipoCandidatura(bsd, usuario, Utilidades.mensajeProperties("constante_modulo_diputadosRP"));
		} catch (Exception e) {
	...
	public boolean existeActaPorCapturar() {
		log.info("Entro a MBCapturaVotoDiputadoRP.existeActaPorCapturar" );

		try {
			form.setExisteActasXcapturar(false);
			// TODO Carga de informacion inicial
			carga = hlp.cargaInformacionInicialCaptura(bsd, bsdCaptura, usuario, candidatura.getTipoCasillas(), Utilidades.mensajeProperties("constante_modulo_diputadosRP"));
			if ( carga == null ) {
	...
	public void inicializaForm() {
		log.info("Entro a MBCapturaVotoDiputadoRP.inicializaForm: " );
		...
		try {
			...
			if ( action.equalsIgnoreCase("idCaptura") ) {
				if ( carga == null ) {
					...
					// TODO Carga las casillas que aun no estan capturadas, las cuales seran las que son suceptibles hacer capturadas
					log.info("PASO 0 else!!!!!!!");
					carga.setCasillasCargaIni(hlp.cargaCasillasNoCaptura(bsdCaptura, carga.getCasillasWS(), usuario, candidatura.getIdTipoCandidatura()));
					if ( carga.getCasillasCargaIni() != null && !carga.getCasillasCargaIni().isEmpty()) {
						// TODO Carga los distritos
						carga.setDistritos(hlp.cargaDistritos(carga.getCasillasCargaIni()));
						log.info("PASO 1!!!!!!!");
						// TODO Precarga la informacion de idDistrito, la seccion, la casilla y estatus
						hlp.precargaDeActaCapturaDipRP(bsd, form, carga, action);
					...
					log.info("lista fdsafdfdsfad: " + carga.getCasillasCargaIni());
				}
			} else {
				// TODO Consultar las actas o casillas capturadas
				carga = hlp.cargaInformacionInicialConsulta(bsd, bsdCaptura, usuario, candidatura.getIdTipoCandidatura(), candidatura.getTipoCasillas(), Utilidades.mensajeProperties("constante_modulo_diputadosRP"));
				if ( carga == null ) {
					agregaMensaje(TipoMensaje.INFO_MENSAJE.getTipo(), "mensajesInfo", Utilidades.mensajeProperties("mensaje_regVotacion_noExistenCasEspeciales"));
				} else {
					if ( carga.getCasillasCargaIni() != null && !carga.getCasillasCargaIni().isEmpty() ) {
						...
						// TODO Cargamos el combo de distritos
						carga.setDistritos(hlp.cargaDistritos(carga.getCasillasCargaIni()));
	...
	/**
	 * Realiza la carga de secciones en base a un distrito seleccionado
	 */
	public void cargaSecciones() {
		log.info("Entro a MBCapturaVotoDiputadoRP.cargaSecciones: " );
		...
		try {
			...
			if ( isCampoDistritoValido() ) {
				carga.setSecciones(hlp.getCargaSeccionesXdistrito(carga.getCasillasCargaIni(), form.getActa().getIdDistrito()));
			} else {
				form.setRequiredDistrito(true);
				form.getActa().setIdDistrito(0);
			}
		} catch (Exception e) {
			// TODO Auto-generated catch block
			form.setActaValida(false);
	...
	public void cargaCasillas() {
		log.info("Entro a MBCapturaVotoDiputadoRP.cargaCasillas " );
		carga.setCasillas(null);
	...
	public void cargaInformacionCasillaCaptura() throws Exception {
		log.info("Entro a MBCapturaVotoDiputadoRP.cargaInformacionCasillaCaptura " );
		form.setShowPanelVotos(false);
		carga.setCandidatos(null);
		showBotonAceptar();
		setRequeridosForm();

		if ( isCampoDistritoValido() && isCampoSeccionValido() && isCampoCveCasillaValido() && isCampoEstatusActaValido() ) {
			obtenSelectItemCasilla();
			...
			cc.setIdDistrito(form.getActa().getIdDistrito());

			// TODO Realizar Calculo del Avance de las casillas
			Object [] calculos = hlp.cargaCalculos(bsdCaptura, usuario, carga.getUniversoTotCasillas(), cc, Utilidades.mensajeProperties("constante_modulo_diputadosRP"));
			carga.setAvanceGlobalActasCapt((BigDecimal)calculos[0]);
			carga.setActasCapturadas((BigDecimal)calculos[1]);
			carga.setActasPendPorCapturar((BigDecimal)calculos[2]);

			// TODO Carga los candidatos
			carga.setCandidatos(hlp.cargaCandidatos(bsd, usuario, form.getActa().getIdDistrito(), null, candidatura.getIdTipoCandidatura()));
	...
	public void cargaInformacionCasillaConsultaModifica() throws Exception {
		log.info("Entro a MBCapturaVotoDiputadoRP.cargaInformacionCasillaConsultaModifica" );
		...
		if ( isCampoDistritoValido() && isCampoSeccionValido() && isCampoCveCasillaValido() ) {
			...
			List<DTOActaCasillaVotos> actaVotos = hlp.getActaCapturada(bsdCaptura, usuario, cc, Utilidades.mensajeProperties("constante_modulo_diputadosRP"));
			
			//CAMBIO 12/06/2017 Por caso de recuento de casillas especiales
			carga.setAvanceGlobalActasCapt((BigDecimal)calculos[0]);
			carga.setActasCapturadas((BigDecimal)calculos[1]);
			carga.setActasPendPorCapturar((BigDecimal)calculos[2]);
			if( actaVotos.get(0).getCapturada()!=null && actaVotos.get(0).getCapturada().intValue()==0 )
			{
				carga.setAvanceGlobalActasCapt(new BigDecimal(0));
				carga.setActasCapturadas(new BigDecimal(0));
				carga.setActasPendPorCapturar(new BigDecimal(1));
			}
			//Fin cambio 
			...
			} else if ( isCampoEstatusActaValido() && !action.equalsIgnoreCase("idCaptura") && form.getActa().getIdEstatusActa().intValue() == Integer.valueOf(Utilidades.mensajeProperties("constante_estatus_recuentoTotal")) ) {
				//CAMBIO: 12/06/2017
				if( form.getActa().getIdEstatusActa().intValue() == Integer.valueOf(Utilidades.mensajeProperties("constante_estatus_recuentoTotal")) )
				{
	...
	public void precargaAvanceCasillas() {
		log.info("-> precargaAvanceCasillas");
		try {
			if ( isCampoDistritoValido() && isCampoSeccionValido() && isCampoCveCasillaValido() ) {
				obtenSelectItemCasilla();
				cc = new DTOClaveCasilla();
	...
	public String guardar() {
		...
		try {
			...
			//TODO Guarda acta
			bsdCaptura.guardar(hlp.formatoEntityGuardar(form.getActa(), carga.getCandidatos(), usuario, Utilidades.mensajeProperties("constante_modulo_diputadosRP")));
			exitoCaptura = true;
			action  = "idConsulta";
			return "consulta";
		} catch ( Exception e ) {
	...
	public String eliminar() {
		...
		try {
			...
			if ( hlp.existeDistribucionOgeneracionActa(bsdCaptura, usuario, cc, Utilidades.mensajeProperties("constante_modulo_diputadosRP"))) {
	...
	/**
	 * Set de los campos requerido en la vista
	 */
	...
	public void showBotonAceptar() throws Exception {
		...
		if ( action.equalsIgnoreCase("idCaptura") ) {
			...
			if ( isCampoDistritoValido()  && isCampoSeccionValido() && isCampoCveCasillaValido() && isCampoEstatusActaValido() ) {
				...
				cc.setIdDistrito(form.getActa().getIdDistrito());
				cc.setIdTipoCandidatura(candidatura.getIdTipoCandidatura());
				// TODO Valida que no se haya realizado la generacion para esa acta.
				if ( hlp.existeDistribucionOgeneracionActa(bsdCaptura, usuario, cc, Utilidades.mensajeProperties("constante_modulo_diputadosRP")) ) {
					form.setActaValida(false);
					form.setActaBloqueada(true);
	...
	/**
	 * Valida que el campo distrito tenga un valor valido. </BR>
	 * true  = si es valido,
	 * false = No es valido
	 * @return
	 */
	public boolean isCampoDistritoValido() {

		if ( form.getActa().getIdDistrito() != null && form.getActa().getIdDistrito() > 0 ) {
			return true;
		}
		return false;
	}
