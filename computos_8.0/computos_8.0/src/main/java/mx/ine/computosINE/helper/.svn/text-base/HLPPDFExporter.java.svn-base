...
public class HLPPDFExporter extends PDFExporter {
    ...
    private PdfPTable creaEncabezadoHistoricos() throws DocumentException {
        ...
        if (parametros.containsKey(Constantes.PARAMETRO_OBJECT_ESTADO)) {
            ...
            encabezado.addCell(cell);
            // ESTADO
            cell = new PdfPCell(new Phrase(estado.getNombreEstado().toUpperCase()));
            cell.setHorizontalAlignment(Element.ALIGN_CENTER);
            cell.setColspan(3);
            encabezado.addCell(cell);
        }
        if (parametros.containsKey(Constantes.PARAMETRO_OBJECT_MUNICIPIO)) {
            ...
            // ETIQUETA
            cell = new PdfPCell(new Phrase(Utilidades.mensajeProperties("etiqueta_sistema_municipio")));
            cell.setHorizontalAlignment(Element.ALIGN_LEFT);
            cell.setColspan(1);
            cell.setBackgroundColor(Color.LIGHT_GRAY);
            encabezado.addCell(cell);
            // ID DISTRITO
            cell = new PdfPCell(new Phrase(municipio.getIdMunicipio().toString()));
            cell.setHorizontalAlignment(Element.ALIGN_CENTER);
            cell.setColspan(1);
            encabezado.addCell(cell);
            // ETIQUETA
            cell = new PdfPCell(new Phrase(Utilidades.mensajeProperties("etiqueta_generales_nombre")));
            cell.setHorizontalAlignment(Element.ALIGN_LEFT);
            cell.setColspan(1);
            cell.setBackgroundColor(Color.LIGHT_GRAY);
            encabezado.addCell(cell);
            // DISTRITO
            cell = new PdfPCell(new Phrase(municipio.getNombreMunicipio().toUpperCase()));
            cell.setHorizontalAlignment(Element.ALIGN_CENTER);
            cell.setColspan(1);
            encabezado.addCell(cell);
        }
