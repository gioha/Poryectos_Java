/**
 * @(#)DTOHBandejaECCPHelper.java 10/01/2018
 *
 ...
 *
 */
public class DTOHBandejaECCPHelper implements Serializable {

	/**
	 * Objeto para la serialización de esta clase.
	 */
	private static final long serialVersionUID = -1252929992903751784L;

	private List<DTOHBandejaECCPEntity> ccp;
	...
	public DTOHBandejaECCPHelper() {
		ccp = new ArrayList<DTOHBandejaECCPEntity>();
	...
	public String obtenerFiltro(boolean conAlias) {
		...
		for (DTOHBandejaECCPEntity elemento : ccp) {
	...
	/**
	 * Método que regesa el método de ordenamiento de la lista Lazy 
	 * 
	 * @return
	 *
	 * @author Homero Fidel Villanueva
	...
	public String obtenerOrdenamiento() {
		...
		if (camposOrdenamiento.size() > 0) {
			...
			for (String campo : camposOrdenamiento) {
				cad += campo + " , ";
			}
	...
	public void ordenarPorFecha( boolean esAscendente) {
		camposOrdenamiento = new ArrayList<String>();
		listaOrden =  new ArrayList<String>();
		agregarCampoOrdenamiento(Utilidades.mensajeProperties("alias_tabla_bandeja_e_ccp") + "." +"FECHA_RECEPCION", true,esAscendente);
	}

	/**
	 * @return valor de tipo List<DTOHBandejaECCPEntity> que esta contenido en la variable ccp
	 *
	 * @author Homero Fidel Villanueva
	 * @since 18/01/2018
	 */
	public List<DTOHBandejaECCPEntity> getCcp() {
		return ccp;
	}
	 ...
	 *            List<DTOBandejaECCPEntity>
	 *
	 * @author Homero Fidel Villanueva
	 * @since 08/12/2017
	 */
	public void setCcp(List<DTOHBandejaECCPEntity> ccp) {
		this.ccp = ccp;
	}
