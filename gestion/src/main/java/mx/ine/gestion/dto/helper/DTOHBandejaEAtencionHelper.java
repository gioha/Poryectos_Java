/**
 * @(#)DTOHBandejaEAtencionHelper.java 19/01/2018
 *
 ...
 *
 */
public class DTOHBandejaEAtencionHelper implements Serializable {

	/**
	 * Objeto para la serialización de esta clase.
	 */
	private static final long serialVersionUID = -2019748557764160930L;

	/**
	 * Atributo que sirve para guardar los filtros para realizar la consulta de
	 * la lista Lazy
	 */
	private List<DTOHBandejaEAtencionEntity> atencion;
	...
	public DTOHBandejaEAtencionHelper() {
		atencion = new ArrayList<DTOHBandejaEAtencionEntity>();
	...
	public String obtenerFiltro(boolean conAlias) {
		...
		if(atencion != null){
			for (DTOHBandejaEAtencionEntity elemento : atencion) {
	...
	public void ordenarPorFecha(boolean esAscendente) {
		camposOrdenamiento = new ArrayList<String>();
		listaOrden =  new ArrayList<String>();
		agregarCampoOrdenamiento(Utilidades.mensajeProperties("alias_tabla_bandeja_e_atencion") + "." +"FECHA_RECEPCION", true, esAscendente);
	}

	/**
	 * @return valor de tipo List<DTOHBandejaEAtencionEntity> que esta contenido en la variable atencion
	 *
	 * @author Homero Fidel Villanueva
	 * @since 19/01/2018
	 */
	public List<DTOHBandejaEAtencionEntity> getAtencion() {
		return atencion;
	}


	/**
	 * @param atencion : valor que se ingresa a la variable de tipo List<DTOHBandejaEAtencionEntity>
	 *
	 * @author Homero Fidel Villanueva
	 * @since 19/01/2018
	 */
	public void setAtencion(List<DTOHBandejaEAtencionEntity> atencion) {
